...
<div class="content">
  <div class="top-bar">
    ...
    <nav aria-label="breadcrumb" class="-intro-x mr-auto hidden sm:flex">
      <ol class="breadcrumb">
        <li class="breadcrumb-item active font-semibold" aria-current="page">Edit Postingan</li>
      </ol>
    </nav>
    ...
    <div class="intro-x dropdown w-8 h-8 mr-24 mb-2">
       <form action="/logout" method="post">
          @csrf
        <button type="submit" class="text-white hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm pl-5  py-2.5 mr- mb-1 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800 w-28 h-9 flex text-xs">Log Out
 ...
 <form action="/dashboard/blogs/{{ $blog->slug }}" method="POST" enctype="multipart/form-data">
           @method('put')
            ...
            <div class="mb-3 grid grid-cols-2 mt-12">
                ...
                <input type="input" class="border-2 mt-2 border-slate-300 outline-slate-400 px-4 rounded-lg col-span-2 h-8 @error('title') is-invalid @enderror id="title name="title"
                    autofocus value="{{ old('title', $blog->title ) }}" autocomplete="off" required>
            ...
            <div class="mb-3 grid grid-cols-2 mt-4">
                ...
                <input type="input" class="border-2 mt-2 border-slate-300 outline-slate-400 px-4 rounded-lg col-span-2 h-8 @error('slug') is-invalid @enderror" id="slug" name="slug"
                    value="{{ old('slug', $blog->slug ) }}" required autocomplete="off">
            ...
            <div class="mb-3">
                ...
                <div class="mb-3">
                    <label for="image" class="form-label" >Post Image</label>
                     <input type="hidden" name="oldImage" value="{{ $blog->image }}">
                       @if($blog->image)
                    <img src="{{ asset ('storage/' . $blog->image) }}" alt="" class="img-preview img-fluid mb-3 col-sm-5 d-block max-w-md">
                    @else
                        <img  class="img-preview w-24 img-fluid mb-3 col-sm-5">
                    @endif
                    <img src="" class="~mb-3 col-sm-5">
                    <input class="border-2 border-slate-300 p-2 w-full  focus:outline-slate-300 outline-slate-400 px-4 rounded-lg  @error('image') is-invalid @enderror" type="file" id="image" name="image"
                     onchange="previewImage()" >
                     @error('image')
                        <div class="invalid-feedback">
                            {{ $message }}
            ...
            <button type="submit" class="p-2 bg-[#1E40AF] text-slate-50 px-4 rounded-lg transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-240  duration-300">Create blog</button>
            <a href="/dashboard/blogs/{{ $blog->slug }}" class="p-[9px] bg-[#1E40AF] text-slate-50 px-10 rounded-lg transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-240  duration-300">Cancel</a>
        </form>
